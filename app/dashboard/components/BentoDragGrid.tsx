...
function SortableCard({
  ...
  const { activeWorkspaceId } = useGeassStore();
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      ...
      <div
        {...attributes}
        {...listeners}
        className={`absolute top-2 left-2 z-30 p-1.5 rounded-lg cursor-grab active:cursor-grabbing
      ...
      <MagicCard
        className={`h-full rounded-[1.25rem] transition-opacity duration-200 ${
...
export default function BentoDragGrid({
  ...
  const [order, setOrder] = useState<string[]>(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        const added = initialWidgets
          .filter((w) => !valid.includes(w.id))
          .map((w) => w.id);
        return [...valid, ...added];
      }
    } catch {}
  ...
  const activeWidget = activeId
    ? initialWidgets.find((w) => w.id === activeId)
    : null;
